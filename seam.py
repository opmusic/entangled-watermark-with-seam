...
def train(x_train, y_train, x_test, y_test, ewe_model, plain_model, epochs, w_epochs, lr, n_w_ratio, factors,
    ...
    #load model
    #saver = tf.train.import_meta_graph('mnist_watermark_model.meta')
    ...
    #forget phase
    forget_index = np.random.choice(index, size=round(0.1*len(index)))
    x_forget = x_train[forget_index]
    y_forget = np.random.randint(0, num_class, size= len(forget_index))
    y_forget = tf.keras.utils.to_categorical(y_forget, num_class)

    model.optimizer = tf.train.AdamOptimizer(5e-5, 0.9, 0.5, 5e-5)
    for epoch in range(5):
        for batch in range(int(len(forget_index)/batch_size)):
                #y_random = np.random.randint(0, num_class, size=batch_size)
                #y_random = tf.keras.utils.to_categorical(y_random, num_class)
                _, temp_grad = sess.run(model.optimize, {x: x_forget[batch * batch_size: (batch + 1) * batch_size],
                                          y: y_forget[batch * batch_size: (batch + 1) * batch_size], w:w_label, t: [1,1,1],
                                          is_training: 1, is_augment: 0})
                #temperatures -= temp_lr * temp_grad[0]
    ...
    for batch in range(num_test):
        ...
        forget_error_list.append(
            sess.run(model.error, {x: x_test[batch * batch_size: (batch + 1) * batch_size], y: true_label,
                                   is_training: 0, is_augment: 0}))
    forget_error = np.average(forget_error_list)

    forget_watermark_acc_list = []
    for batch in range(w_num_batch):
        forget_watermark_acc_list.append(validate_watermark(
            model, trigger[batch * half_batch_size: (batch + 1) * half_batch_size], watermark_target))
    forget_watermark_acc = np.average(forget_watermark_acc_list)
    if verbose:
        print(f"Forget Model || validation accuracy: {1 - forget_error},"
              f" watermark success: {forget_watermark_acc}")

    # Recover Phase
    recover_index = np.random.choice(index, size=round(0.01*len(index)))
    x_recover = x_train[recover_index]
    y_recover = y_train[recover_index]

    model.optimizer = tf.train.GradientDescentOptimizer(1e-3)
    for epoch in range(10):
        for batch in range(int(len(recover_index)/batch_size)):
            _, temp_grad = sess.run(model.optimize, {x: x_recover[batch * batch_size: (batch + 1) * batch_size],
                                      y: y_recover[batch * batch_size: (batch + 1) * batch_size],
                                      w: w_label, t: [1,1,1],
                                      is_training: 1, is_augment: 0})
            #temperatures -= temp_lr * temp_grad[0]
    ...
    recover_error_list = []
    for batch in range(num_test):
        recover_error_list.append(sess.run(model.error, {x: x_test[batch * batch_size: (batch + 1) * batch_size],
                                                          y: y_test[batch * batch_size: (batch + 1) * batch_size],
                                                          is_training: 0, is_augment: 0}))
    recover_error = np.average(recover_error_list)

    recover_list = []
    for batch in range(w_num_batch):
        recover_list.append(validate_watermark(
            model, trigger[batch * half_batch_size: (batch + 1) * half_batch_size], watermark_target))
    recover_watermark = np.average(recover_list)

    if verbose:
        print(f"Recover Model || validation accuracy: {1 - recover_error}, "
              f"watermark success: {recover_watermark}")


    return 1 - victim_error, victim_watermark_acc, 1 - forget_error, forget_watermark_acc, 1 - recover_error, \
           recover_watermark
